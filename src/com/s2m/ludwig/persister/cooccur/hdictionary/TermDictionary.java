...
public class TermDictionary implements Dictionary {
	...
	public TermDictionary() throws IOException {
	...
	public long convert(String term) {
		Long value = (Long) mcc.get(term);
		//long value = mcc.getCounter(term);
	...
	private long insert(String term) {
		long value;
		try {
			...
			if(!htable.checkAndPut(rowkey, DICTIONARY_COLUMN_FAMILY, DICTIONARY_COLUMN, null, p)) {
				LOG.warn("Concurrent insert, we lost a counter with term "+ term);
				value = getLong(term);
			}
		} catch (IOException e) {
	...
	private long getLong(String term) {
		long value = 0;
		Result res;
